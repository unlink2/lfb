...
void lfb_args_free(void) {
  void *argtable[] = lfb_argtable;
  arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
}

void lfb_args_parse(int argc, char **argv) {
...
int main(int argc, char **argv) {
  lfb_args_parse(argc, argv);
  
  // map args to cfg here 
  struct lfb_config cfg;
  memset(&cfg, 0, sizeof(cfg));

  cfg.verbose = verb->count > 0;

  int res = lfb_main(&cfg);
